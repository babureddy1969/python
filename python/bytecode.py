...
#The Bytes Type
#The bytes type in Python is immutable and stores a sequence of values ranging from 0-255 (8-bits). 0
# You can get the value of a single byte by using an index like an array, 
# but the values can not be modified.
# Create empty bytes
empty_bytes = bytes(4) #b'\x00\x00\x00\x00'
...
with open("test.txt", "ab") as binary_file:
    # Write text or bytes to the file
    binary_file.write("Write text by encoding\n".encode('utf8'))
    #num_bytes_written = binary_file.write(b'\xDE\xAD\xBE\xEF')
    #print("Wrote %d bytes." % num_bytes_written)
...
four_bytes = i.to_bytes(4, byteorder='big', signed=True) #b'\x00\x00\x00\x10'
print(four_bytes)

# Compare the difference to little endian
print(i.to_bytes(4, byteorder='little', signed=True)) #b'\x10\x00\x00\x00'
...
text = binary_data.decode('utf-8') # I am text.
print(text)
